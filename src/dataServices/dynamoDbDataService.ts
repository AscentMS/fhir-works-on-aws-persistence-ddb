...
export class DynamoDbDataService implements Persistence, BulkDataAccess {
    ...
    private async createResourceWithId(resourceType: string, resource: any, resourceId: string, tenantId?: string) {
        ...
        try {
            await this.dynamoDb.putItem(param);
        } catch (e) {
        ...
        const item = unmarshall(param.Item);
    ...
    async deleteVersionedResource(resourceType: string, id: string, vid: number, tenantId?: string) {
        ...
        await this.dynamoDb.updateItem(updateStatusToDeletedParam);
        return {
            success: true,
    ...
    async getJobsWithExportStatuses(jobStatuses: ExportJobStatus[]): Promise<Record<string, AttributeValue>[]> {
        const jobStatusPromises = jobStatuses.map((jobStatus: ExportJobStatus) => {
            ...
            return this.dynamoDb.query(queryJobStatusParam);
        });

        const jobStatusResponses = await Promise.all(jobStatusPromises);
        let allJobStatusItems: Record<string, AttributeValue>[] = [];
    ...
    async cancelExport(jobId: string, tenantId?: string): Promise<void> {
        ...
        const jobDetailsResponse = await this.dynamoDb.getItem(jobDetailsParam);
        if (!jobDetailsResponse.Item) {
            throw new ResourceNotFoundError('$export', jobId);
        }
        const jobItem = unmarshall(jobDetailsResponse.Item);
    ...
    async getExportStatus(jobId: string, tenantId?: string): Promise<GetExportStatusResponse> {
        ...
        const item = unmarshall(jobDetailsResponse.Item);
